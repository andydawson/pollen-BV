...
preds <- readRDS('/scratch/pollen-model/polya-gamma-predictions_5.0_overdispersed.RDS')
...
keep_idx = sample(1000, 200)

rerun = TRUE

for (tx in 1:dim(preds[["pi"]])[3]){
  print(paste0("Taxon: ", tx))
  if (rerun)#(!file.exists(paste0('output/',taxa[tx],'_bvs_n200_v5.0.RDS')))
  {
    txpreds = preds[["pi"]][keep_idx,,tx,]
    ...
    rasterstack_list <- mclapply(1:n_iter,mc.cores=8,function(i)
    {
    ...
    for(i in 1:n_iter){
      rasterstack_rev[[i]] <- as(rasterstack_rev[[i]], 'SpatRaster')
    }
    
    # calc_bv = function(x) {
    #   enmSdmX::bioticVelocity(x, 
    #                           times=(rev(time$time_mid)* -1), 
    #                                 onlyInSharedCells=FALSE) #false to match climate
    # }
    # 
    # 
    # 
    # bar = as(rasterstack_rev[[1]], 'SpatRaster')
    # foo = calc_bv(bar)
    # 
    # bar = as(rasterstack_rev[[1]], 'SpatRaster')
    
    ##parallelized calculate BVs
    bv_list = mclapply(rasterstack_rev, mc.cores=8, function(x) {
      enmSdmX::bioticVelocity(x, 
                              times=(rev(time$time_mid)* -1), 
                              onlyInSharedCells=FALSE) #false to match climate
    })
    ...
    saveRDS(bvs, paste0('output/',taxa[tx],'_bvs_n200_v5.0.RDS'))
